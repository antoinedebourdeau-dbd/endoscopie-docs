...
cp -R "$ROOT/index.html" "$ROOT/css" "$ROOT/js" "$ROOT/img" "$ROOT/vendor" "$ROOT/chu-logo.webp" "$ROOT/favicon.png" \
      "$ROOT/manifest.webmanifest" "$ROOT/sw.js" "$ROOT"/icon-*.png "$STAGE/"
...
# le service worker suit la version de l'app (invalide l'ancien cache) — source + staging
sed -i '' "s/^const VERSION = \"[^\"]*\";/const VERSION = \"$V\";/" "$ROOT/sw.js" "$STAGE/sw.js"
